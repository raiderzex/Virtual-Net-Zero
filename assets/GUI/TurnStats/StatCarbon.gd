...
func _process(delta):
	...
	if (Global.embodiedCarbon + Global.operationalCarbon) >= 250000:
		self.text = str("Your total carbon cost for the year " + str(Global.turn_count + 2023) + " is " +
		str(Global.embodiedCarbon + Global.operationalCarbon) + " kgCO2eq. Try harder and work towards carbon net-zero!" )
	elif (Global.embodiedCarbon + Global.operationalCarbon) <= 250000 and (Global.embodiedCarbon + Global.operationalCarbon) > 0:
		self.text = str("Your total carbon cost for the year " + str(Global.turn_count + 2023) + " is " +
		str(Global.embodiedCarbon + Global.operationalCarbon) + " kgCO2eq. Good job! You're getting close to carbon net-zero!" )
