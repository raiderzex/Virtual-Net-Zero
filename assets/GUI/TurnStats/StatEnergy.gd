...
func _process(delta):
	...
	if (Global.energyConsumption) >= 250000:
		self.text = str("Your energy consumption for the year " + str(Global.turn_count + 2023) + " is " +
		str(Global.energyConsumption) + " kWh. Try harder to achieve net-zero energy!" )
	elif (Global.energyConsumption) <= 250000 and (Global.energyConsumption) > 0:
		self.text = str("Your energy consumption for the year " + str(Global.turn_count + 2023) + " is " +
		str(Global.energyConsumption) + " kWh. Good job! You're getting close to net-zero energy!" )	
